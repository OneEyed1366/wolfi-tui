...
import type { Alien, Bullet, Shield } from '../hooks/useInvaders'
...
function renderRow(row: Cell[], rowIndex: number): React.ReactNode {
	...
	return (
		<Box key={`row-${rowIndex}`}>
			{segments.map((seg) => (
				<Text key={seg.key} className={seg.color || undefined}>
					{seg.text}
				</Text>
			))}
		</Box>
...
function GameCanvasInner({
	...
	const renderedRows = useMemo(() => {
		...
		return canvas.map((row, y) => renderRow(row, y))
	}, [aliens, bullets, shields, playerX, frame, boardWidth, boardHeight])

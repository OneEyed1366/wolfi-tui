...
export function mapCSSProperty(
	...
	if (normalizedProp === 'flex') {
		...
		if ('flexShrink' in expanded)
			result['flexShrink'] = expanded.flexShrink as number
		if ('flexBasis' in expanded)
			result['flexBasis'] = expanded.flexBasis as number | string
	...
	if (normalizedProp === 'border') {
		...
		for (const part of parts) {
			...
			if (
				part.startsWith('#') ||
				part.startsWith('rgb') ||
				CSS_NAMED_COLORS.has(part.toLowerCase())
			) {
	...
	if (normalizedProp === 'color') {
		return { color: parseColor(trimmedValue) }
	}

	if (
		normalizedProp === 'background-color' ||
		normalizedProp === 'background'
	) {
		// For 'background', only handle solid colors
		if (
			normalizedProp === 'background' &&
			(trimmedValue.includes('gradient') || trimmedValue.includes('url'))
		) {
			return null
		}
	...
	if (
		normalizedProp === 'text-decoration' ||
		normalizedProp === 'text-decoration-line'
	) {
		if (trimmedValue.includes('underline')) {
			return { textDecoration: 'underline' }
		}
		if (trimmedValue.includes('line-through')) {
	...
	// inverse
	if (normalizedProp === 'inverse') {
		return { inverse: trimmedValue === 'true' }
	}

 ...
 */
function parseArgs(args: string[]): { options: CLIOptions; inputFiles: string[] } {
 ...
 */
async function processFile(inputFile: string, options: CLIOptions): Promise<void> {
	...
	const css = await compile(source, lang, absolutePath);

	// Parse CSS to styles
	const styles = parseCSS(css, { filename: inputFile });
...
async function main(): Promise<void> {
	...
	if (options.mode !== 'module' && options.mode !== 'global') {
		console.error(`Error: Invalid mode "${options.mode}". Use "module" or "global".`);
		process.exit(1);
	}
